...
def log_start(task: str, env: str, model: str, is_eval: bool = True) -> None:
    if is_eval: print(f"[START] task={task} env={env} model={model}", flush=True)
    console.print(f"\n[bold cyan]>> STARTING TASK:[/bold cyan] [yellow]{task}[/yellow] | Model: [green]{model}[/green]")

def log_step(step: int, action: str, reward: float, done: bool, error: str = None, reasoning: str = None, is_eval: bool = True) -> None:
    error_val = error if error else "null"
    done_val = str(done).lower()
    if is_eval: print(f"[STEP] step={step} action={action} reward={reward:.2f} done={done_val} error={error_val}", flush=True)
...
def log_end(task: str, score: float, steps: int, is_eval: bool = True) -> None:
    if is_eval: print(f"[END] task={task} score={score:.4f} steps={steps}", flush=True)
...
async def run_simulation(task_name, defender_type="llm", prompt_sys=None, seed=42):
    ...
    is_eval_agent = (defender_type == "llm")
    log_start(task=task_name, env=BENCHMARK, model=MODEL_NAME if is_eval_agent else defender_type, is_eval=is_eval_agent)
...
Note: The environment contains a rule-based deterministic attacker. Threats build up, and your visibility of them is limited strictly to turns where you scan."""
    ...
    for step in range(1, 20):  # max_steps is 15 but we can loop to 20 safely
        ...
        log_step(step, act_str, reward, done, error_val, reasoning_str, is_eval=is_eval_agent)
    
    res_dict = {"score": getattr(env, f"grade_{task_name}")(), "metrics": env.get_grade_breakdown(task=task_name)}
    log_end(task=task_name, score=res_dict["score"], steps=env.step_count, is_eval=is_eval_agent)
